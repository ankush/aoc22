...
fn part1(input: &str) -> i32 {
    ...
    rucksacks
        .iter()
        .map(|r| find_common_character(r.left, r.right))
        .map(get_priority)
        .sum()
}

fn find_common_character(a: &str, b: &str) -> char {
    a.chars().filter(|c| b.contains(*c)).next().unwrap()
}
...
fn get_rucksacks(input: &str) -> Vec<Rucksack> {
    ...
    for line in input.lines() {
        ...
        rucksacks.push(Rucksack { left, right })
    }
...
CrZsJsPPZsGzwwsLwLmpwMDw";
    ...
    fn common_char() {
        ...
        {
            let actual_common_char = find_common_character(rucksack.right, rucksack.left);
